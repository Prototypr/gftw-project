...
Strapi is an Open Source headless CMS that acts as the backend of the Prototypr app.

In a 'headless CMS' setup, the front-end is completely separated from the back end, ao there are 2 parts of the Prototypr app, both in different GitHub Repos:

* **Prototypr front-end** – a [NextJS app](https://github.com/GraemeFulton/prototypr-next) that displays our data
* **Prototypr backend** – [A Strapi Content Management System](https://github.com/GraemeFulton/prototypr-backend) that stores and handles all the data.
...
In our Strapi repository, there's a folder called `/wordpress-migration`, which has a `README.md` on how to migrate our content from Wordpress. It reads content from different post types using the WP-GraphQL endpoints, and inserts them into Strapi with a Post request. The outcome looks like this:
...
3. Set it up on the basic tier ($10 a month), with the following **12 Environmental variables**:
...
```
These are created automatically with any new Strapi project. (todo: see if there's a generator for this file)
